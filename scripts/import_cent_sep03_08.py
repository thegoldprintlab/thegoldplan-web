#!/usr/bin/env python3
"""Import Cent MT5 history screenshots 2026-09-03 afternoon → 2026-09-08.

Source: Telegram iPhone screenshots (11 images, tesseract OCR).
PnL is COMPUTED: (close-open)*volume*100 for buy, (open-close)*volume*100 for sell.
Do NOT use OCR profit (minus signs drop on some crops).

Idempotent:
  - DELETE Cent rows with trade_date >= 2026-09-04
  - DELETE Cent rows on 2026-09-03 whose entry matches the afternoon batch
    (morning 09-03 trades already in DB are kept)
  - INSERT the uniq OCR rows
"""
import json
import os
import sys
...
SRC = os.path.join(ROOT, ".extract_cent_sep08", "_all_uniq.json")
ACCOUNT = "Cent"
...
def fetch_all(token, uid, url, anon, extra_q=""):
    all_rows = []
    offset = 0
    limit = 1000
    while True:
        q = (
            f"/rest/v1/trades?user_id=eq.{uid}&account=eq.Cent{extra_q}"
...
def main():
    ...
    afternoon_entries = sorted({
        i["entry_price"] for i in ins if i["trade_date"] == "2026-09-03"
    })
    print("09-03 afternoon entries:", afternoon_entries)

    if not commit:
        print("\n--- DRY RUN (first 8 + last 8) ---")
        for i in ins[:8] + ins[-8:]:
            print(
    ...
    sep3 = [r for r in before if r["trade_date"] == "2026-09-03"]
    to_del_ids = []
    for r in sep3:
        if float(r["entry_price"]) in afternoon_entries:
            to_del_ids.append(r["id"])
    ge04 = [r for r in before if r["trade_date"] >= "2026-09-04"]
    to_del_ids.extend(r["id"] for r in ge04)
    print(f"[delete] sep3-afternoon={len([r for r in sep3 if float(r['entry_price']) in afternoon_entries])} "
          f">=09-04={len(ge04)} ids={len(to_del_ids)}")

    for i in to_del_ids:
        st, txt, _ = req(
            "DELETE", f"/rest/v1/trades?id=eq.{i}&user_id=eq.{uid}",
            token=tok, extra={"Prefer": "return=minimal"}, url=URL, anon=ANON,
        )
        if st >= 300:
            print("DELETE FAIL", st, txt[:200], i)
            sys.exit(1)
    ...
    print("Cent dates last 10:", dict(sorted(by.items())[-10:]))
    recent = [r for r in after if r["trade_date"] >= "2026-09-03"]
    print(f"Cent >=09-03: {len(recent)} net={round(sum(float(r['profit_loss']) for r in recent), 2)}")
    byr = Counter()
    pnlr = Counter()
    for r in recent:
        byr[r["trade_date"]] += 1
        pnlr[r["trade_date"]] += float(r["profit_loss"])
    for d in sorted(byr):
